// Bihon addition, MPL-2.0. Magnify the reader without resizing its page layout.
...
export function ReaderZoom({ children }: { children: ReactNode }) {
    ...
    const apply = useCallback((next: { scale: number; x: number; y: number }) => {
        ...
        content.style.transform = value.scale === 1 ? 'none' : `translate(${value.x}px, ${value.y}px) scale(${value.scale})`;
    ...
    useEffect(() => {
        ...
        const isControl = (target: EventTarget | null) => target instanceof Element && !!target.closest('[data-bihon-zoom-toolbar]');
        ...
        const wheel = (event: WheelEvent) => {
            ...
            if (event.ctrlKey || event.metaKey) {
                stop(event);
                const pixels = event.deltaY * (event.deltaMode === 1 ? 16 : event.deltaMode === 2 ? frame.clientHeight : 1);
                zoomAt(position.current.scale * Math.exp(-clamp(pixels, -120, 120) * 0.003), event.clientX, event.clientY);
                ...
                if (x !== current.x || y !== current.y) { stop(event); apply({ ...current, x, y }); }
            }
        };
        const down = (event: PointerEvent) => {
            if (position.current.scale <= 1 || event.button !== 0 || isControl(event.target)) return;
            stop(event);
        ...
        const move = (event: PointerEvent) => {
            ...
            stop(event);
            apply({ ...position.current, x: position.current.x + event.clientX - drag.x, y: position.current.y + event.clientY - drag.y });
        ...
        const click = (event: MouseEvent) => {
            if (position.current.scale > 1 && !isControl(event.target)) stop(event);
        };
        const key = (event: KeyboardEvent) => {
            const target = event.target;
            if (target instanceof HTMLElement && (target.isContentEditable || /^(INPUT|TEXTAREA|SELECT)$/.test(target.tagName))) return;
            if (event.key === 'Escape' && position.current.scale > 1) { stop(event); reset(); }
            if (!event.ctrlKey && !event.metaKey) return;
            if (['+', '=', '-', '0'].includes(event.key)) {
                stop(event);
                if (event.key === '0') reset();
                else zoomAt(position.current.scale * (event.key === '-' ? 1 / 1.2 : 1.2));
            }
        };
        const menu = (event: Event) => {
            const action = (event as CustomEvent<string>).detail;
            if (action === 'reset') reset();
            else if (action === 'in' || action === 'out') zoomAt(position.current.scale * (action === 'in' ? 1.2 : 1 / 1.2));
        };
    ...
    return (
        <Box ref={viewport} data-testid="reader-zoom-viewport" sx={{ position: 'relative', width: '100%', height: '100%', minHeight: 0, flex: 1, overflow: 'hidden' }}>
            <Box ref={surface} data-testid="reader-zoom-surface" data-zoom="1" sx={{ width: '100%', height: '100%', transformOrigin: '0 0' }}>{children}</Box>
            <Paper data-bihon-zoom-toolbar role="toolbar" aria-label="Panel zoom" elevation={4}
                title="Ctrl+wheel or pinch to zoom. Drag or scroll to pan. Escape or Ctrl+0 resets."
                sx={{ position: 'absolute', right: 16, bottom: 76, zIndex: 1400, display: 'flex', alignItems: 'center', borderRadius: 2 }}>
                <IconButton aria-label="Zoom out of manga" disabled={scale <= 1} onClick={() => zoomAt(position.current.scale / 1.2)}><RemoveIcon /></IconButton>
                <Button aria-label="Reset manga zoom" onClick={reset} sx={{ minWidth: 62, fontVariantNumeric: 'tabular-nums' }}>{Math.round(scale * 100)}%</Button>
                <IconButton aria-label="Zoom into manga" disabled={scale >= MAX_ZOOM} onClick={() => zoomAt(position.current.scale * 1.2)}><AddIcon /></IconButton>
            </Paper>
        </Box>
    );
}
